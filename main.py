...
async def handle_consumabels_selection(message: types.Message, state: FSMContext):
    ...
    if consumable == 'Далее':
        ...
        if consumable in consumables:
            ...
            await state.update_data(consumables=consumables)
            await state.update_data(current_cons = consumable)
            await state.set_state(CleaningFSM.consumabels_amoutn)
            await message.answer(
                f"Укажите количество для расходника '{consumable}', пожалуйста, введите число.",
                reply_markup=None 
            )

@dp.message(CleaningFSM.consumabels_amoutn)
async def handle_consumabels_amount(message: types.Message, state: FSMContext):
    ...
    data = await state.get_data()
    current_cons = data.get("current_cons")
    consumables = data.get("consumables", {})

    consumables[current_cons] = amount
    await state.update_data(consumables=consumables)
